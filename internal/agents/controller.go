...
// +kubebuilder:rbac:groups=capture.gateway.io,resources=captureplugins,verbs=get;list;watch
...
func (r *TrafficCaptureReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Step 2: Check if this is a plugin-based capture.
	var plugin *capturev1alpha1.CapturePlugin
	if tc.Spec.PluginRef != nil {
		plugin = &capturev1alpha1.CapturePlugin{}
		pluginKey := types.NamespacedName{Namespace: tc.Namespace, Name: string(tc.Spec.PluginRef.Name)}
		if err := r.Get(ctx, pluginKey, plugin); err != nil {
			logger.Error(err, "CapturePlugin not found", "plugin", pluginKey)
			return r.setPhaseWithCondition(ctx, tc, capturev1alpha1.TrafficCapturePhaseFailed,
				"PluginNotFound", fmt.Sprintf("CapturePlugin %s not found: %v", pluginKey, err))
		}
	}

	// Step 3: Create/update Deployment (plugin-based or built-in agent).
	...
	// Step 6: Add mirror filter to target route (skip for standalone plugins).
	if plugin == nil || plugin.Spec.Mode == capturev1alpha1.CapturePluginModeMirror {
